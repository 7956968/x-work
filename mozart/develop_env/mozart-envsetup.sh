...
function mozart-create-release()
{
    ...
    if [ $# -lt 1 ]; then
        ...
        if [ "$T" ]; then
            for project in $D; do $T/develop_env/repo forall $project -c "git remote add local git@192.168.2.2:/backup/02Bu1shared/01SmartAudio_Release/codes/`basename $project`"; done
...
function mozart-merge-release()
{
    ...
    if [ $# -lt 1 ]; then
        ...
        if [ "$T" ]; then
            mozart-git "branch -D speaker-release-tmp"
            mozart-git "checkout -b speaker-release-tmp --track ing/speaker-release"
            mozart-git "pull"
            mozart-git "merge --no-ff release-$1"
            mozart-git "tag $1 -m \"$1\""
