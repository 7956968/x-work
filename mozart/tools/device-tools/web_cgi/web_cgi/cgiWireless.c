...
//get first ssid in wpa_supplicant.conf
int wpa_get_ssid(char* ssid, int ssid_len)
{
	FILE* fp = fopen("/usr/data/wpa_supplicant.conf", "r");
	if (!fp) {
		mylogstrTofile("%s: open wpa_supplicant failed\n", __FUNCTION__);
		return -1;
	}
	char str[128] = {0};
	while(fgets(str, 128, fp)) {
		char* tmp1 = NULL;
		mylogstrTofile("%s: file content:%s\n", __FUNCTION__, str);
		
		if ( NULL == (tmp1 = strstr(str, "ssid=\""))  || (tmp1 != str)) 
			continue;
		tmp1 += strlen("ssid=\"");
		mylogstrTofile("%s: %s\n", __FUNCTION__, tmp1);
		char* tmp2 = NULL;
		if ( NULL == (tmp2 = strchr(tmp1, '"')) )
			continue;
		mylogstrTofile("%s: %s\n", __FUNCTION__, tmp2);
		*tmp2 = '\0';
		strncpy(ssid, tmp1, ssid_len-1);
		fclose(fp);
		return 0;	
	}
	fclose(fp);
	mylogstrTofile("%s: could not found ssid\n", __FUNCTION__);
	return -1;
