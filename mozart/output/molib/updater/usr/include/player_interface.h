...
#define CH_CNT 2
	...
	typedef struct {
		...
		char uuid[32];
		/**
		 * @brief channel of current controller.
		 */
		int ch_num;
		/**
		...
		int duration;
		/**
		 * @brief player status changed by seek, Should check it on (status == PLAYER_PLAYING).
		 */
		bool change_by_seek;
	} player_snapshot_t;
	...
	/**
	 * @brief apply a handler from player, only for channel 0, compatible with old version
	 *
	 ...
	 *
	 * @param uuid [in] uuid of your module, will store in player_handler_t->uuid, limit to 32Bytes.
	 * @param ch [in] channels number you want to control, 0 or 1, used to start new stream.
	 * @param callback [in] callback on player status change.
	 * @param param [in] private data, will be passed as the last argument of callback.
	 *
	 * @return return player handler on success, return NULL otherwise.
	 */
	extern player_handler_t *mozart_player_ch_handler_get(char *uuid, int ch, player_callback callback, void *param);
	...
	/**
	 * @brief switch audio output device
	 *
	 * @param handler player handler
	 * @param ao target ao (specific reference mplayer -ao command, for example oss:/dev/dsp
	 * alsa:noblock:device=hw=0.0 or pcm:file=*.wav)
	 *
	 * @return return 0 on success, return -1 otherwise
	 */
	extern int mozart_player_aoswitch(player_handler_t *handler, char *ao);
	/**
