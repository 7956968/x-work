...
static char *ingenicplayer_get_volume(void)
{
	json_object *reply_object;
	char *reply_json = NULL;
...
static void ingenicplayer_get_queue(char *command, struct appserver_cli_info *owner)
{
	int i = 0;
	int index = -1;
	int length = -1;
...
static char *ingenicplayer_add_queue(char *data)
{
	int index = -1;
	...
	return ingenicplayer_get_song_info();
}

static char *ingenicplayer_replace_queue(char *data, struct appserver_cli_info *owner)
{
	int i = 0;
	int index = -1;
	int length = -1;
...
static char *ingenicplayer_play_queue(char *data)
{
	int index = -1;
...
static char *ingenicplayer_get_device_info(void)
{
	int volume;
	char *devicename = NULL;
	char *reply_json = NULL;
...
static char *ingenicplayer_get_sd_music(void)
{
	char *sdmusic = NULL;
	char *reply_json = NULL;
	json_object *songs = NULL;
...
static int ingenicplayer_handle_command(struct cmd_info_c *cmd_info)
{
	...
	case INGENICPLAYER_GET_VOLUME:
		reply_json = ingenicplayer_get_volume();
