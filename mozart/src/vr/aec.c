...
int mozart_aec_start(void)
{
	...
	aec_stop_flag = false;
	aec_running = true;
#if (SUPPORT_VR_WAKEUP == VR_WAKEUP_VOICE_KEY_MIX || SUPPORT_VR_WAKEUP == VR_WAKEUP_VOICE)//HZB add
...
#endif
		aec_feed(dmic_buf, loopback_buf, AEC_SIZE);
#else 
		(void)(&dmic_buf==&loopback_buf);
		(void)(&soundcard_aec_record==&soundcard_aec_record);
		(void)(&soundcard_uninit==&soundcard_uninit);
		(void)(&soundcard_init==&soundcard_init);
		usleep(300000);
#endif
	}
