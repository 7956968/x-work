...
static int update_info_parse_updatefiles_times(char *versionfile, struct nv_info *info)
{
	int i = 0;
	int offset = 0;
	...
	for (i = 0; i < cnt; i++) {
		...
		if (strcmp(filenames[i], "uboot")) {
			/* updater, kernel, appfs will auto calc offset by static order */
			...
			strcpy((char *)info->files[i].location, (char *)info->files[i].blkdev);
		}
		pr_err("%s will be at %s +%#llx.\n", info->files[i].name, (char *)info->files[i].blkdev, info->files[i].offset);
		free(filenames[i]);
	}
...
static int update_info_parse_updatefiles_once(char *versionfile, struct nv_info *info)
{
	int i = 0;
	int cnt = UPDATE_FILE_MAX;
	char **filenames = NULL;
	...
	for (i = 0; i < cnt; i++) {
		...
		info->files[i].size = atoi(size_buf);
		/* updater, kernel, appfs will auto calc offset by static order */
		if (!strcmp(filenames[i], "kernel")) {
			int blkid = 0;
			int blkid_pos = 0;
			info->files[i].offset = 0;
			strcpy((char *)info->files[i].location, (char *)info->update_method.location);
			/* FIXME: partitions cnt >= 10 */
			blkid_pos = strlen(info->files[i].location) - 1;
			blkid = atoi(info->files[i].location + blkid_pos);
			sprintf(info->files[i].location + blkid_pos, "%d", blkid + 1);
		} else if (!strcmp(filenames[i], "uboot")) {
			info->files[i].offset = 0;
			strcpy((char *)info->files[i].location, (char *)info->files[i].blkdev);
		} else {
			info->files[i].offset = 0;
		...
		pr_err("%s will be at %s(from %s) +%#llx.\n", info->files[i].name, (char *)info->files[i].blkdev, info->files[i].location, info->files[i].offset);
		free(filenames[i]);
	}
	free(filenames);
...
static int update_info_parse_updatefiles(char *versionfile, struct nv_info *info)
{
	...
	if (info->update_method.method == UPDATE_ONCE) {
		ret = update_info_parse_updatefiles_once(versionfile, info);
	} else if (info->update_method.method == UPDATE_TIMES) {
		ret = update_info_parse_updatefiles_times(versionfile, info);
	}
...
struct nv_info *mozart_updater_chkver(void)
{
	...
	// if has new version already, return directly.
	// have new version, return true
	if (has_new_version(info))
		goto return_direct;

	// else check new version from update server.
...
static int erase_location(struct nv_info *info, char *filename)
{
	struct otafile_info f;
...
static int update_info_download_updatefiles(struct nv_info *info)
{
	...
	if (info->update_method.method == UPDATE_ONCE) {
		if (info->update_process == PROCESS_1) {
			erase_location(info, "updater");
			erase_location(info, "kernel");
...
int mozart_updater_update_uboot(struct nv_info *info)
{
	int ret = 0;
	struct otafile_info f;

	f = get_otafile_info(info, "uboot");

	if (strcmp(f.name, "uboot")) {
		pr_info("No new uboot found, ignore it!\n");
		return 0;
	}

	erase_blk((char *)f.location);

	ret = update_info_download_updatefile(info, "uboot");
