...
#define PCMDIV_N_MAX 0x1fff
#define PCMDIV_N_MIN 1
#define PCMDIV_M_MAX 0x1ff
#define PCMDIV_M_MIN 1
#define PCMDIV_CLKDIV 32
#define MULI 100000
#define DIV_PCM 100
#define SAMPLE_SIZE_16BIT 1
#define SAMPLE_SIZE_8BIT 0
...
{
	if (*channel >= 5) {
		printk("pcm don't support 5 channel now.\n");
		return -1;
	}
	return 0;
}
...
static int pcm_set_rate(unsigned long *rate)
{
	...
	if (pcm_priv->pcm_device_mode == PCM_MASTER) {
		__pcm_set_syndiv(((pcm_priv->pcmclk / pcm_priv->rate) / 8)-1);
		__pcm_set_clkdiv(PCMDIV_CLKDIV - 1);
		if (!pcm_priv)
			return -ENODEV;
		div = pcm_priv->pcmclk/(8*(*rate)) - 1;
		if (div >= 0 && div < 32) {
	//		__pcm_set_syndiv(div);
	//		__pcm_set_syndiv((PCMDIV_SYNDIV/8)-1);
...
static int cal_err(int m ,int n)
{
	unsigned int tmp0;
	...
	DE_N = 50;
	tmp0 = n * MULI / m;
	err_max = (div_max * MULI - tmp0) * 1000 / tmp0;
	err_min = (tmp0 - div_min * MULI) * 1000 / tmp0;
	if((err_max < DE_P) && (err_min < DE_N))
		return 0;
	else
		return -1;
}
static unsigned long get_cpm_pcm_clk(void)
{
	return pcm_priv->pcmclk * PCMDIV_CLKDIV;
}
static int pcm_div_clk(unsigned long clk,int *pcmdiv_m,int *pcmdiv_n)
{
	int i;
	unsigned int M, N,P = 0,Q = 0;
	int m_max, n_max;
	int tmp_err = 0;
	int max_err = clk / 4000;
	m_max = 0x1ff;
	n_max = 0x1fff;
	for (i = 1; i < m_max; i++) {
		M = i;
		N = (pcm_priv->cpm_pll_clk / DIV_PCM * M) / (clk / DIV_PCM);
		if (N == 0) {
			return -1;
		}
		if (N > n_max)
		...
		tmp_err = abs((pcm_priv->cpm_pll_clk / DIV_PCM * M) / N * DIV_PCM - clk);
		if (!cal_err(M,N))
		{
			if (tmp_err < max_err) {
				max_err = tmp_err;
...
static int pcm_clk_init(struct platform_device *pdev)
{
	...
	int pcmdiv_m = 0,pcmdiv_n = 0;
	unsigned long getclk = 0;
	getclk = get_cpm_pcm_clk();
	pll_clk = clk_get(&pdev->dev,"apll");
	if (IS_ERR(pll_clk)) {
		printk(KERN_ERR"pcm get mpll_clk fail\n");
		goto __err_pll_clk_get;
	}
	pcm_priv->cpm_pll_clk = clk_get_rate(pll_clk);
	pcm_div_clk(getclk,&pcmdiv_m,&pcmdiv_n);
	...
	if (pcm_priv->pcm_device_mode == PCM_MASTER) {
		__pcm_as_master();
#if 1
		audio_write( (pcmdiv_m << 13) | (pcmdiv_n),PCMCDR_PRE );
		*(volatile unsigned int*)0xb00000e0 = *(volatile unsigned int*)0xb00000e0;
		audio_write( (pcmdiv_m << 13) | (pcmdiv_n) | (1 << 29) ,PCMCDR_PRE );
#else
	...
	if ((pcm_priv->pcm_slot_num < 5) && (pcm_priv->pcm_slot_num > 0))
		__pcm_set_slot(((pcm_priv->pcm_slot_num-1) & 0x3));
	else
		printk("pcm: slot num is 1 2 3 or 4 .\n");
