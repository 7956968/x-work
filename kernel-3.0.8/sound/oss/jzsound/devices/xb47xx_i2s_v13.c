...
{	/*to be modify*/
	struct codec_info *tmp = kzalloc(sizeof(struct codec_info), GFP_KERNEL);
...
static int i2s_set_rate(struct i2s_device * i2s_dev, unsigned long *rate,int mode)
{
	...
	printk("i2s_set_rate: rate(%ld) mode(%d) codec_mode(%d)\n", *rate, mode, cur_codec->codec_mode);
	if (mode & CODEC_WMODE) {
		...
		if (cur_codec->codec_mode == CODEC_SLAVE) {
			...
			if(ret < 0) {
				printk("ERROR: set i2s replay rate failed\n");
				return -EINVAL;
			}
			#if 1
			//HZB add, when sample rate == 8khz, i treat that as if system is answering bt call, in which case the mclk must be 22Mhz.
			//so i2sdiv = 22M/64/8K=48=0x30
			#ifdef CONFIG_AK4425_4104_EXTERNAL_CODEC
			if (*rate == 24000) {
				printk("set i2sdiv to 0x10\n");
				audio_write(0x10, I2SDIV_PRE);
			} else
			#endif
			#endif
			...
			*(volatile unsigned int*)0xb0000070 = 0x0;
			...
			if(ret < 0) {
				printk("ERROR: set codec mclk failed\n");
				return -EINVAL;
			}
			*(volatile unsigned int*)0xb0000070 = 0x0;
		}
	...
	if (mode & CODEC_RMODE) {
		...
		if (cur_codec->codec_mode == CODEC_SLAVE) {
			...
			ret = clk_set_rate(i2s_dev->i2s_clk, sysclk);
			if(ret < 0) {
				printk("ERROR: set i2s record rate failed\n");
				return -EINVAL;
			}
			audio_write(0x3, I2SDIV_PRE);
			*(volatile unsigned int*)0xb0000070 = 0x0;
...
static void i2s_set_trigger(struct i2s_device * i2s_dev, int mode)
{
	...
	if (mode &CODEC_RMODE) {
		...
		if (I2S_RX_FIFO_DEPTH < burst_length/data_width){
			printk("Error: burst_length is exceed the RX FIFO depth.\n");
			__i2s_set_receive_trigger(i2s_dev, ((I2S_RX_FIFO_DEPTH + 1) >> 1) - 1);
		} else {
...
static int i2s_reset(int mode,struct codec_info *cur_codec,struct i2s_device *i2s_dev)
{
	if (!cur_codec)
		return -ENODEV;
	__i2s_stop_bitclk(i2s_dev);
	...
	if(!strcmp(cur_codec->name, "i2s_external_codec")) {
		__i2s_external_codec(i2s_dev);
	}else if(!strcmp(cur_codec->name,  "internal_codec")) {
...
static void i2s_set_spdifdeinit()
{
	__spdif_disable_transmit_dma();
	__interface_select_i2s();
	__spdif_disable();
}
#endif
...
static int i2s_enable(struct i2s_device * i2s_dev, int mode)
{
	...
	if (!dp_other->is_used) {
#ifdef CONFIG_SOUND_JZ_SPDIF_V13
		i2s_reset(mode,cur_codec,i2s_dev);
#endif
...
//	dev_info("WARNING: unless one codec must be select for i2s.\n");
	...
	if(!i2s_dev->i2s_work_queue_1) {
		printk("create i2s workqueue error!\n");
		ret = -ENOMEM;
		goto err_workqueue;
	}
...
struct i2s_device *get_g_i2s_device(void)
{
    return g_i2s_dev;//g_i2s_dev define in xb47xx_i2s_v13.c
}
