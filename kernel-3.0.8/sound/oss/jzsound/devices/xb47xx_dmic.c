...
static struct clk *dmic_clk_tmp = NULL;
/*##################################################################*\
 | dump
\*##################################################################*/
...
{
	struct codec_info_dmic *tmp = kzalloc(sizeof(struct codec_info_dmic), GFP_KERNEL);
...
int dmic_set_rate(unsigned long *rate,int mode)
{
	...
	ret = clk_set_rate(dmic_clk_tmp,24000000);
	if(ret < 0) {
		printk("ERROR: dmic set rate failed!\n");
		return -EINVAL;
	}
	if (mode & CODEC_WMODE) {
		/*************************************************\
...
int dmic_init_pipe(struct dsp_pipe **dp , enum dma_data_direction direction,unsigned long iobase)
{
	...
	*dp = kmalloc(sizeof(struct dsp_pipe), GFP_KERNEL);
	...
	if (direction == DMA_FROM_DEVICE) {
		...
		(*dp)->fragsize = FRAGSIZE_S;
		(*dp)->fragcnt = FRAGCNT_S;
		(*dp)->dma_config.src_maxburst = 32;
		(*dp)->dma_config.dst_maxburst = 32;
		(*dp)->dma_config.dst_addr = iobase + DMICDR;
...
int dmic_global_init(struct platform_device *pdev)
{
	...
	dmic_clk_tmp = clk_get(&pdev->dev, "cgu_i2s");
	if (IS_ERR(dmic_clk_tmp)) {
		dev_err(&pdev->dev, "----> cgu_i2s clk_get failed\n");
		goto __err_dmic_clk;
	}
	clk_enable(dmic_clk_tmp);
#ifdef DMIC_IRQ
	/* request irq */
