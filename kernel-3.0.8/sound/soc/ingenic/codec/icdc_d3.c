/*
 * sound/soc/ingenic/codec/icdc_d3.c
...
static char amixer_set_mute = 0;
static char stream_set_mute = 0;
...
static int icdc_d3_digital_mute(struct snd_soc_dai *dai, int mute)
{
	...
	/* 
	 * Here we have two controller operation the same DAC mute register independently.
	 * So we should add the flag to coordinate their work.
	 */
	if (mute){
		stream_set_mute = 1;
	} else {
		stream_set_mute = 0;
		if (amixer_set_mute == 1)
			return 0;
	}
...
static int jz_icdc_startup(struct snd_pcm_substream *substream,struct snd_soc_dai *dai)
{
	struct snd_soc_codec *codec = dai->codec;
	/*power on codec*/
	snd_soc_update_bits(codec, SCODA_REG_CR_VIC, SCODA_CR_VIC_SB_MASK, 0);
	if (snd_soc_update_bits(codec, SCODA_REG_CR_VIC, SCODA_CR_VIC_SB_SLEEP_MASK, 0))
		msleep(400);
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE)
		snd_soc_update_bits(codec, SCODA_REG_CR_ADC, SCODA_CR_ADC_SMUTE_MASK, 0 << SCODA_CR_ADC_SMUTE_SHIFT);
	return 0;
}


static void jz_icdc_shutdown(struct snd_pcm_substream *substream,struct snd_soc_dai *dai)
{
	struct snd_soc_codec *codec = dai->codec;
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE)
		snd_soc_update_bits(codec, SCODA_REG_CR_ADC, SCODA_CR_ADC_SMUTE_MASK, 1 << SCODA_CR_ADC_SMUTE_SHIFT);
	/*power off codec*/
	snd_soc_update_bits(codec, SCODA_REG_CR_VIC, SCODA_CR_VIC_SB_SLEEP_MASK, 1);
	snd_soc_update_bits(codec, SCODA_REG_CR_VIC, SCODA_CR_VIC_SB_MASK, 1);
	return;
...
/* This private interface is just for digital playback mute operation */
#define INGENIC_SOC_SINGLE(xname, xreg, xshift, xmax, xinvert) \
...
/* unit: 0.01dB */
static const DECLARE_TLV_DB_SCALE(dac_tlv, -3100, 0, 0);
static const DECLARE_TLV_DB_SCALE(mix_tlv, -3100, 0, 0);
...
static const struct snd_kcontrol_new icdc_d3_snd_controls[] = {
	...
	SOC_SINGLE_TLV("Mic Volume", SCODA_REG_GCR_MIC1, 0, 31, 0, mic_tlv),

	/* ADC private controls */
	SOC_SINGLE("ADC High Pass Filter Switch", SCODA_REG_FCR_ADC, 6, 1, 0),
	/* Digital mute controls */
	INGENIC_SOC_SINGLE("Digital Playback mute", SCODA_REG_CR_DAC, 7, 1, 0),
...
static int icdc_d3_platform_remove(struct platform_device *pdev)
{
	...
	platform_set_drvdata(pdev, NULL);
	device_remove_file(&pdev->dev, &icdc_d3_sysfs_attrs);
	return 0;
}
