				...
				struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	if((jz_i2s->i2s_mode & I2S_MASTER)&&(jz_aic->sample_rate != sample_rate)){
		...
		jz_aic->sysclk = clk_get_rate(jz_aic->clk);
		if(jz_aic->sysclk > codec_sysclk)
			printk("set i2s sysclk failed!!\n");
	}
	return 0;
}

static int jz_i2s_start_substream(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	struct jz_i2s *jz_i2s = dev_get_drvdata(dai->dev);
	struct device *aic = jz_i2s->aic;
...
static int jz_i2s_stop_substream(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	struct jz_i2s *jz_i2s = dev_get_drvdata(dai->dev);
	struct device *aic = jz_i2s->aic;
