 ...
 *
 * Charger driver for RICOH R5T618 power management chip.
 *
 ...
 *
 */
#define RICOH618_BATTERY_VERSION "RICOH618_BATTERY_VERSION: 2015.3.31 V3.1.1.3"
...
/* FG setting */
#define RICOH618_REL1_SEL_VALUE		64
...
#define RICOH618_DISPLAY_UPDATE_TIME		15
...
#define RICOH618_UPDATE_COUNT_FULL_RESET 	7
#define RICOH618_CHARGE_UPDATE_TIME		3
#define RICOH618_FULL_WAIT_TIME			4
#define RE_CAP_GO_DOWN				10	/* 40 */
#define RICOH618_ENTER_LOW_VOL			0
#define RICOH618_TAH_SEL2			5
#define RICOH618_TAL_SEL2			6
#define RICOH618_LOAD_C				1200

#define RICOH618_OCV_OFFSET_BOUND	3
#define RICOH618_OCV_OFFSET_RATIO	2

#define RICOH618_ENTER_FULL_STATE_OCV	9
#define RICOH618_ENTER_FULL_STATE_DSOC	90
...
static int calc_capacity_2(struct ricoh618_battery_info *info);
...
static int get_power_supply_Android_status(struct ricoh618_battery_info *info);
...
static int get_battery_temp_2(struct ricoh618_battery_info *info);
static int check_jeita_status(struct ricoh618_battery_info *info, bool *is_jeita_updated);
static void ricoh618_scaling_OCV_table(struct ricoh618_battery_info *info, int cutoff_vol, int full_vol, int *start_per, int *end_per);
static int ricoh618_Check_OCV_Offset(struct ricoh618_battery_info *info);
...
static int set_Rlow(struct ricoh618_battery_info *info)
{
	int err;
	int Rbat_low_max;
	uint8_t val;
	...
	if (Vocv < get_OCV_voltage(info,3))
	{
		...
		val = temp >> 8;
		err = ricoh618_write_bank1(info->dev->parent, 0xD4, val);
		if (err < 0) {
		...
		err = ricoh618_write_bank1(info->dev->parent, 0xD5, val);
		if (err < 0) {
			dev_err(info->dev, "PMU:batterry initialize error\n");
			return err;
		}
...
static int Set_back_ocv_table(struct ricoh618_battery_info *info)
{
	int err;
	uint8_t val;
	int temp;
	...
	if (0 != info->soca->ocv_table_low[0])
	{
		...
		err = ricoh618_clr_bits(info->dev->parent, FG_CTRL_REG, 0x01);

		err = ricoh618_bulk_writes_bank1(info->dev->parent,
			BAT_INIT_TOP_REG, 22, info->battery_init_para[info->num]);

		err = ricoh618_set_bits(info->dev->parent, FG_CTRL_REG, 0x01);

		/* debug comment start*/
		err = ricoh618_bulk_reads_bank1(info->dev->parent,
...
static int set_cc_sum_back(struct ricoh618_battery_info *info, int *val)
{
	...
	err = ricoh618_set_bits(info->dev->parent, CC_CTRL_REG, 0x01);
	if (err < 0)
		return -1;
	...
	/* CC_pause exit */
	err = ricoh618_clr_bits(info->dev->parent, CC_CTRL_REG, 0x01);
	if (err < 0)
		return -1;
...
static int set_pswr(struct ricoh618_battery_info *info)
{
	int val;
	int err;
...
static int ricoh618_Check_OCV_Offset(struct ricoh618_battery_info *info)
{
	...
	pr_info("PMU : %s : calc ocv %d get OCV %d\n",__func__,calc_ocv(info),get_OCV_voltage(info, RICOH618_OCV_OFFSET_BOUND));
	...
	if (val != 0){ /* connect adp or usb */
		if (calc_ocv(info) < get_OCV_voltage(info, RICOH618_OCV_OFFSET_BOUND) )
		{
			if(0 == info->soca->ocv_table_low[0]){
				for (i = 0 ; i < 11; i++){
				...
				for (i = 0 ; i < 11; i++){
					temp = ocv_table[i] * (100 + RICOH618_OCV_OFFSET_RATIO) / 100;

					info->battery_init_para[info->num][i*2 + 1] = temp;
					info->battery_init_para[info->num][i*2] = temp >> 8;
				}
				ret = ricoh618_clr_bits(info->dev->parent, FG_CTRL_REG, 0x01);

				ret = ricoh618_bulk_writes_bank1(info->dev->parent,
					BAT_INIT_TOP_REG, 22, info->battery_init_para[info->num]);

				ret = ricoh618_set_bits(info->dev->parent, FG_CTRL_REG, 0x01);

				/* debug comment start*/
				ret = ricoh618_bulk_reads_bank1(info->dev->parent,
					BAT_INIT_TOP_REG, 22, debug_disp);
				for (i = 0; i < 11; i++){
					pr_info("PMU : %s : after OCV table %d 0x%x\n",__func__, i * 10, (debug_disp[i*2] << 8 | debug_disp[i*2+1]));
				}
...
static int reset_FG_process(struct ricoh618_battery_info *info)
{
	int err;

	//err = set_Rlow(info);
	//err = ricoh618_Check_OCV_Offset(info);
...
static int check_charge_status_2(struct ricoh618_battery_info *info, int displayed_soc_temp)
{
	...
	if (POWER_SUPPLY_STATUS_FULL == info->soca->chg_status) {
		if ((info->first_pwon == 1)
			&& (RICOH618_SOCA_START == info->soca->status)) {
				g_full_flag = 1;
...
**/
static int calib_ocvTable(struct ricoh618_battery_info *info, int vbat_ocv)
{
	int ret;
	int cutoff_ocv;
	int i;
	...
	if (info->soca->Ibat_ave > RICOH618_REL1_SEL_VALUE) {
	...
	ricoh618_scaling_OCV_table(info, cutoff_ocv/1000, ocv100_new/1000, &start_per, &end_per);
...
static void ricoh618_displayed_work(struct work_struct *work)
{
	...
	if ((RICOH618_SOCA_START == info->soca->status)
		 ...
		 || (RICOH618_SOCA_FULL == info->soca->status))
		{
	...
	err = ricoh618_read(info->dev->parent, CHGSTATE_REG, &val);
	if (err < 0) {
		dev_err(info->dev, "PMU:Error in reading the control register\n");
		return;
	}
	...
	ricoh618_write(info->dev->parent, 0xc2, 0x00);
	ricoh618_write(info->dev->parent, 0xc3, 0x00);

	local_irq_save(flags);
	if((val > 0) && (info->suspend_lock_locked == 0)) {
		printk("displayed is charging wake lock\n");
		wake_lock(&info->suspend_lock);
		info->suspend_lock_locked = 1;
	} else if((val == 0) && (info->suspend_lock_locked == 1)){
		printk("displayed no charging wake unlock\n");
		wake_unlock(&info->suspend_lock);
		wake_lock_timeout(&info->suspend_lock, HZ);
		info->suspend_lock_locked = 0;
	}
	...
	if (info->flag_get_charge_work <=0 || info->flag_get_charge_work > 5)
	{
		if (info->flag_get_charge_work <=0) {
			pr_info("PMU: get_charge_work abnormal hangup, restarting...\n");
		...
		else{ pr_info("PMU: get_charge_work, Read V/I directly Due to Sleep. \n");}
	...
	if ((RICOH618_SOCA_DISP == info->soca->status)
		...
		if(g_full_flag == 1){	/* for issue 1 solution start*/
			...
			if ((calculated_ocv > get_OCV_voltage(info, RICOH618_ENTER_FULL_STATE_OCV))
				|| (POWER_SUPPLY_STATUS_FULL == info->soca->chg_status)
				|| (info->soca->displayed_soc > RICOH618_ENTER_FULL_STATE_DSOC * 100)) {
				info->soca->status = RICOH618_SOCA_FULL;
				g_full_flag = 0;
				info->soca->last_cc_sum = 0;
	...
	if (RICOH618_SOCA_FULL == info->soca->status) {
		...
		if (info->soca->Ibat_ave >= -20 && info->battery_is_really_charging) { /* charging */
			if (0 == info->soca->jt_limit) {
				if (g_full_flag == 1) {
					...
					if (POWER_SUPPLY_STATUS_FULL == info->soca->chg_status) {
						if(info->soca->full_reset_count < RICOH618_UPDATE_COUNT_FULL_RESET) {
							info->soca->full_reset_count++;
						} else if (info->soca->full_reset_count < (RICOH618_UPDATE_COUNT_FULL_RESET + 1)) {
							...
							info->soca->rsoc_ready_flag =1;
							goto end_flow;
						} else if(info->soca->full_reset_count < (RICOH618_UPDATE_COUNT_FULL_RESET + 2)) {
							info->soca->full_reset_count++;
							info->soca->fc_cap = 0;
							info->soca->soc_full = info->soca->soc;
						}
					...
					if ((POWER_SUPPLY_STATUS_FULL == info->soca->chg_status)
						...
						if(info->soca->chg_cmp_times > RICOH618_FULL_WAIT_TIME) {
							info->soca->displayed_soc = 100*100;
							g_full_flag = 1;
							info->soca->full_reset_count = 0;
							info->soca->soc_full = info->soca->soc;
						...
						|| (info->soca->Ibat_ave < info->ch_icchg*50 + 75) )
						{
							...
							/* Calibration for Big mistake 20160126*/
							if (info->soca->displayed_soc < 7500){
                                                                info->soca->displayed_soc = 9000;
                                                                err = reset_FG_process(info);
                                                                if (err < 0)
                                                                        dev_err(info->dev, "Error in writing the control register\n");
                                                                info->soca->rsoc_ready_flag =1;
			...
			if (info->soca->displayed_soc >= 9950) {
				if (info->soca->Ibat_ave <= -1 * RICOH618_REL1_SEL_VALUE) {
					...
					ricoh618_read(info->dev->parent, CHGSTATE_REG, &reg_val);
					if (reg_val & 0xc0) {
						info->soca->displayed_soc = 100 * 100;
					} else {
						g_full_flag = 0;
...
end_flow:
	...
	else if(g_fg_on_mode==0 )  // Ross Function
	{
		...
		if (RICOH618_SOCA_ZERO == info->soca->status)
		{
			// Do not reset CC
			//
		}
		else if(cc_cap > 100)
			{
				...
				pr_info("PMU:after init_pswr is %d \n",info->soca->init_pswr);
	...
	printk("PMU:STATUS= %d: IBAT= %d: VSYS= %d: VBAT= %d: DSOC= %d: RSOC= %d:\n",
	       info->soca->status, info->soca->Ibat_ave*20/info->fg_rsense_val, info->soca->Vsys_ave,
			info->soca->Vbat_ave, info->soca->displayed_soc, info->soca->soc);
	/*
	int i;
	for (i=0xb0;i<0xf0;i++)
	{
		err = ricoh618_read(info->dev->parent, i, &val);
		pr_info("    ----- 0x%x = %x \n",i,val);
	}
	*/
//      err = ricoh618_read(info->dev->parent, 0xbd, &val);
...
static int ricoh618_init_fgsoca(struct ricoh618_battery_info *info)
{
	...
	printk("============in %s,%d:first_pwon=%d\n",__func__,__LINE__,info->first_pwon);
	...
	if (info->first_pwon) {
		...
		err = ricoh618_write(info->dev->parent, CHGISET_REG, 0);
		if (err < 0)
			dev_err(info->dev,
		...
		printk("============in %s,%d:before calc_capacity_in_period\n",__func__,__LINE__);
		err = calc_capacity_in_period(info, &cc_cap, &is_charging, true);
...
*/
{
	...
	*is_jeita_updated = false;
	...
	get_power_supply_status(info);
//	pr_info("PMU: %s *** Hope updating value in this timing after checking jeita,
//		chg_status: %d, is_jeita_updated: %d ******\n", __func__,
//			info->soca->chg_status, *is_jeita_updated);
...
static int ricoh618_factory_mode(struct ricoh618_battery_info *info)
{
	...
	ret = ricoh618_set_bits(info->dev->parent, CHGCTL1_REG, 0x40);
...
//		info->soca->cutoff_ocv = info->soca->target_vsys - Ibat_min * info->soca->Rsys / 1000;
	...
	if ((info->soca->target_ibat == 0) || (info->soca->target_vsys == 0)) {	/* normal version */
		...
		ricoh618_scaling_OCV_table(info, info->soca->cutoff_ocv, full_ocv, &start_per, &end_per);
	...
	/////////////////////////////////
	ret = ricoh618_read_bank1(info->dev->parent, 0xDC, &val);
	if (ret < 0) {
		dev_err(info->dev, "PMU:batterry initialize error\n");
		goto err;
	}
	...
	ret = ricoh618_write_bank1(info->dev->parent, 0xDC, val);
	if (ret < 0) {
		dev_err(info->dev, "PMU:batterry initialize error\n");
		goto err;
	}

	ret = ricoh618_read_bank1(info->dev->parent, 0xDC, &val2);
	if (ret < 0) {
		dev_err(info->dev, "PMU:batterry initialize error\n");
		goto err;
	}

	ret = ricoh618_write_bank1(info->dev->parent, 0xDC, val_temp);
	if (ret < 0) {
		dev_err(info->dev, "PMU:batterry initialize error\n");
		goto err;
	}
	...
	if (val != val2) {
		...
		ret = ricoh618_read_bank1(info->dev->parent, 0xD2, &val);
		if (ret < 0) {
		dev_err(info->dev, "PMU:batterry initialize error\n");
		goto err;
		}

		ret = ricoh618_read_bank1(info->dev->parent, 0xD3, &val2);
		if (ret < 0) {
			dev_err(info->dev, "PMU:batterry initialize error\n");
			goto err;
		}
		...
		if (available_cap_ori == available_cap) {
			...
			for (i = 0; i < 6; i++) {
				ret = ricoh618_write_bank1(info->dev->parent, 0xD4+i, info->battery_init_para[info->num][24+i]);
				if (ret < 0) {
					dev_err(info->dev, "PMU:batterry initialize error\n");
					return ret;
				}
...
static int ricoh618_init_battery(struct ricoh618_battery_info *info)
{
	...
	ret = ricoh618_read_bank1(info->dev->parent, BAT_REL_SEL_REG, &val);
...
//	ret = ricoh618_write(info->dev->parent, FG_CTRL_REG, 0x11);
	ret = ricoh618_write(info->dev->parent, FG_CTRL_REG, 0x51);
	if (ret < 0) {
		dev_err(info->dev, "PMU:Error in writing the control register\n");
		return ret;
	}
...
#endif
	ret = ricoh618_read(info->dev->parent, POWEROFF_HISTORY_REG, &reg_val);
	if (ret < 0) {
		dev_err(info->dev,"PMU:Error in reading POWEROFF_HISTORY_REG %d\n", ret);
		return ret;
	}
	printk("power off history:%d\n",reg_val);
	ret = ricoh618_read(info->dev->parent, POWERON_HISTORY_REG, &reg_val);
	if (ret < 0) {
		dev_err(info->dev,"PMU:Error in reading POWERON_HISTORY_REG %d\n", ret);
		return ret;
	}
	printk("power on history:%d\n",reg_val);


	ret = ricoh618_write(info->dev->parent, VINDAC_REG, 0x0);
	if (ret < 0) {
		dev_err(info->dev, "PMU:Error in writing the control register\n");
		return ret;
	}
...
static int ricoh618_init_charger(struct ricoh618_battery_info *info)
{
	...
	if (err < 0) {
		dev_err(info->dev, "PMU:Error in writing RICOH618_RE_CHARGE_VOLTAGE %d\n",
									 err);
		goto free_device;
	}
...
//	info->soca->OCV100_min,info->soca->OCV100_max,vfchg_val,icchg_val,rbat);
	...
	ricoh618_write(info->dev->parent, RICOH618_ADC_CNT1, 0x12);
	/* Set VSYS threshold low voltage value = (voltage(V)*255)/(3*2.5) */
	val = info->alarm_vol_mv * 255 / 7500;
	ricoh618_write(info->dev->parent, RICOH618_ADC_VSYS_THL, val);
...
static void charger_irq_work(struct work_struct *work)
{
	...
	if(info->chg_ctr & 0x02) {
		uint8_t sts;
		ret = ricoh618_read(info->dev->parent, RICOH618_INT_MON_CHGCTR, &sts);
		if (ret < 0)
			dev_err(info->dev, "PMU:Error in reading the control register\n");
	...
	case	1: // plug in ADP
			...
			//ret = ricoh618_write(info->dev->parent, REGISET1_REG, info->adp_current_val);
			break;
	case	2:// plug in USB
			pr_info("%s : val = %d plug in USB\n",__func__, val);
			//Add the code of USB Charge and Limit current settings
			ret = ricoh618_write(info->dev->parent, REGISET2_REG, info->usb_current_val);
			break;
	...
	if((val > 0) && (info->suspend_lock_locked == 0)) {
		printk("Is charging wake lock\n");
		wake_lock(&info->suspend_lock);
		info->suspend_lock_locked = 1;
	} else if((val == 0) && (info->suspend_lock_locked == 1)){
		printk("No charging wake unlock\n");
		wake_unlock(&info->suspend_lock);
		wake_lock_timeout(&info->suspend_lock, HZ);
		info->suspend_lock_locked = 0;
	}
...
static int set_charge_priority(struct ricoh618_battery_info *info, bool *data)
{
	...
	if (*data == 1)
		...
		ret = ricoh618_clr_bits(info->dev->parent, CHGCTL1_REG, val);
...
static int calc_capacity_2(struct ricoh618_battery_info *info)
{
	...
	if (info->soca->rsoc_ready_flag  != 0) {
		...
		if (fa_cap != 0) {
			...
			ret = ricoh618_read(info->dev->parent, SOC_REG, &val);
			if (ret < 0) {
				dev_err(info->dev, "PMU:Error in reading the control register\n");
				return ret;
			}
...
static int measure_vbatt_ADC(struct ricoh618_battery_info *info, int *data)
{
	...
	ret = ricoh618_set_bits(info->dev->parent, INTEN_REG, 0x08);
	if (ret < 0) {
	...
	ret = ricoh618_set_bits(info->dev->parent, EN_ADCIR3_REG, 0x01);
	if (ret < 0) {
		dev_err(info->dev, "PMU:Error in setting the control register bit\n");
		goto err;
	}
	...
	/* single request */
	ret = ricoh618_write(info->dev->parent, ADCCNT3_REG, 0x10);
	if (ret < 0) {
		dev_err(info->dev, "PMU:Error in writing the control register\n");
		goto err;
	}
	...
	for (i = 0; i < 5; i++) {
		...
		ret = ricoh618_read(info->dev->parent, EN_ADCIR3_REG, &data_h);
		if (ret < 0) {
			dev_err(info->dev, "PMU:Error in reading the control register\n");
			goto err;
		}
...
static enum power_supply_property ricoh618_power_props[] = {
...
struct power_supply	powerac = {
		...
		.properties = ricoh618_power_props,
		.num_properties = ARRAY_SIZE(ricoh618_power_props),
		.get_property = ricoh618_batt_get_prop,
};
...
{
	struct ricoh618_battery_info *info;
	struct ricoh618_battery_platform_data *pdata = dev_get_platdata(pdev->dev.parent);
	int type_n;
	int ret, temp;
	int val;

	pr_info("PMU:PMU: %s : version is %s\n", __func__,RICOH618_BATTERY_VERSION);
	...
	wake_lock_init(&info->suspend_lock, WAKE_LOCK_SUSPEND, "ricoh618-charger");
	...
	if (ret < 0) {
		dev_err(info->dev,
			 "PMU:ricoh618_battery_probe: Error in reading PSWR_REG %d\n", ret);
	}
	val &= 0x7f;
	pr_info("PMU:ricoh618_battery_probe : pswr = %d\n", val);
	...
	info->fg_rsense_val = pdata->type[type_n].fg_rsense_val;
	...
	*/
	...
	info->battery.external_power_changed
		 = ricoh618_external_power_changed;
...
#ifdef ENABLE_FACTORY_MODE
	pr_info("PMU:ENABLE_FACTORY_MODE ...\n");
	...
	info->monitor_wqueue
		= create_singlethread_workqueue("ricoh618_battery_monitor");
	info->workqueue = create_singlethread_workqueue("r5t618_charger_in");
	INIT_WORK(&info->irq_work, charger_irq_work);
	INIT_DELAYED_WORK_DEFERRABLE(&info->monitor_work,
					 ricoh618_battery_work);
	...
	ret = request_threaded_irq(charger_irq + RICOH618_IRQ_FCHGCMPINT, NULL,
						charger_complete_isr,
						IRQF_ONESHOT, "r5t618_charger_comp",
								info);
	if (ret < 0) {
	...
	ret = request_threaded_irq(charger_irq + RICOH618_IRQ_FONCHGINT,
					NULL, charger_in_isr, IRQF_ONESHOT,
						"r5t618_charger_in", info);
	if (ret < 0) {
	...
	ret = request_threaded_irq(charger_irq + RICOH618_IRQ_FVUSBDETSINT,
	...
	ret = request_threaded_irq(charger_irq + RICOH618_IRQ_FVADPDETSINT,
...
#ifdef ENABLE_LOW_BATTERY_DETECTION
	ret = request_threaded_irq(charger_irq + RICOH618_IRQ_VSYSLIR,
					NULL, adc_vsysl_isr, IRQF_ONESHOT,
						"r5t618_adc_vsysl", info);
	if (ret < 0) {
		dev_err(&pdev->dev,
...
static int __devexit ricoh618_battery_remove(struct platform_device *pdev)
{
	...
	if (g_fg_on_mode
		...
		if (err < 0)
			dev_err(info->dev, "PMU:Error in writing PSWR_REG\n");
		g_soc = 0x7f;
	} else if (info->soca->status != RICOH618_SOCA_START
		&& info->soca->status != RICOH618_SOCA_UNSTABLE
		&& info->soca->rsoc_ready_flag != 0) {
	...
	val &= 0x7f;
	pr_info("PMU:ricoh618_battery_remove : pswr = %d\n", val);
	...
	free_irq(charger_irq + RICOH618_IRQ_FCHGCMPINT, &info);
	free_irq(charger_irq + RICOH618_IRQ_FONCHGINT, &info);
	free_irq(charger_irq + RICOH618_IRQ_FVUSBDETSINT, &info);
	free_irq(charger_irq + RICOH618_IRQ_FVADPDETSINT, &info);
#ifdef ENABLE_LOW_BATTERY_DETECTION
	free_irq(charger_irq + RICOH618_IRQ_VSYSLIR, &info);
#endif
#ifdef	ENABLE_FUEL_GAUGE_FUNCTION
	...
	wake_lock_destroy(&info->suspend_lock);
...
static int power_off(struct platform_device *pdev) {
	uint8_t   cc_sum_reg[4] = {0, 0, 0, 0};
	int err;

	err = ricoh618_set_bits(pdev->dev.parent, CC_CTRL_REG, 0x01);
	if(err)
		printk("Error when clean the FG CC_SUM reg!!\n");

	err = ricoh618_bulk_writes(pdev->dev.parent,
			CC_SUMREG3_REG, 4, cc_sum_reg);

	if(err)
		printk("Error when clean the FG CC_SUM reg!!\n");

	err = ricoh618_clr_bits(pdev->dev.parent, CC_CTRL_REG, 0x01);
...
static int ricoh618_battery_suspend(struct device *dev)
{
	...
	pr_info("PMU:resume : displayed_soc = %d , soc = %d \n", info->soca->displayed_soc, info->soca->soc);
	...
	if (info->soca->status == RICOH618_SOCA_DISP
		|| info->soca->status == RICOH618_SOCA_STABLE
		|| info->soca->status == RICOH618_SOCA_FULL) {
		info->soca->soc = calc_capacity_2(info);
		info->soca->soc_delta =
...
/* Initial setting of charger interrupt  */
static int ricoh618_init_charge_interrupt(struct ricoh618_battery_info *info)
{
	int err;
	uint8_t val;
...
static int ricoh618_battery_resume(struct device *dev)
{
	...
	if (info->entry_factory_mode) {
		...
		if (RICOH618_SOCA_START == info->soca->status
			|| RICOH618_SOCA_UNSTABLE == info->soca->status
			|| info->soca->rsoc_ready_flag == 0) {
			ret = calc_capacity_in_period(info, &cc_cap,
							 &is_charging, false);
		} else {
	...
	//ricoh618_init_charge_interrupt(info);
	return 0;
}

static const struct dev_pm_ops ricoh618_battery_pm_ops = {
...
MODULE_DESCRIPTION("RICOH R5T618 Battery driver");
