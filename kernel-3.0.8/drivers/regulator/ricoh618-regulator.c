...
static unsigned int ricoh618_suspend_status;
		...
		int min_uV, int max_uV, unsigned *selector)
{
	...
	if (ricoh618_suspend_status)
		return -EBUSY;

	if(ri->id != RICOH618_ID_DC1_SLP)
		return __ricoh618_set_voltage(parent, ri, min_uV, max_uV, selector);
	else
		return __ricoh618_set_s_voltage(parent, ri, min_uV, max_uV);
}
...
static struct ricoh618_regulator ricoh618_regulator[] = {
	...
	RICOH618_REG(DC1_SLP, 0x2C, 0, 0x2C, 1, 0x36,
			0xFF, 0x3B, 600, 3500, 12500, 0xE8,
			ricoh618_ops, 500),
...
static inline struct ricoh618_regulator *find_regulator_info(int id)
{
	struct ricoh618_regulator *ri;
	int i;

	for (i = 0; i < ARRAY_SIZE(ricoh618_regulator); i++) {
		ri = &ricoh618_regulator[i];
		if (ri->desc.id == id)
			return ri;
	}
	return NULL;
...
static int ricoh618_regulator_preinit(struct device *parent,
		struct ricoh618_regulator *ri,
		struct ricoh618_regulator_platform_data *ricoh618_pdata)
{
	int ret = 0;

	if (!ricoh618_pdata->init_apply)
		return 0;

	if (ricoh618_pdata->init_uV >= 0) {
		ret = __ricoh618_set_voltage(parent, ri,
				ricoh618_pdata->init_uV,
				ricoh618_pdata->init_uV, 0);
		if (ret < 0) {
        ...
        if (ricoh618_pdata->init_uV >= 0) {
		ret = __ricoh618_set_s_voltage(parent, ri,
				ricoh618_pdata->init_uV,
				ricoh618_pdata->init_uV);
                // printk("The voltage %d is set in sleep is %d\n",ri->desc.id,ricoh618_pdata->init_uV);
		if (ret < 0) {
			dev_err(ri->dev, "Not able to initialize voltage %d "
				"for rail %d err %d\n", ricoh618_pdata->init_uV,
				ri->desc.id, ret);
...
{
	struct ricoh618_regulator *ri = NULL;
	...
	err = ricoh618_regulator_preinit(pdev->dev.parent, ri, tps_pdata);
	if (err) {
...
MODULE_DESCRIPTION("RICOH RN5T618 regulator driver");
