...
struct timespec gpio_intr_time;
static int
...
#define NSECS_PER_SEC		1000000000
	...
	msg->flags = htol32(msg->flags);
		...
		if (cmd == WLC_GET_VAR && buf)
		{
			if (!strcmp((char *)buf, "dot1as_bus_delay")) 
			{			
				dot1as_bus_delay_t *bus_delay = (dot1as_bus_delay_t *)(buf);
				bool_isbusdelay = TRUE;
				getnstimeofday(&ts1);
				Magic_num = jiffies;
				bus_delay->sys_nsec_t4 = Magic_num;
			}
		}
#endif
...
retry:
	...
	if (bool_isbusdelay)
	{
		...
		if(!strncmp(bus_delay->name, "dot1as_bus_delay", strlen("dot1as_bus_delay")))
		{	
			if( bus_delay->sys_nsec_t4 == Magic_num)
			{
				bus_delay->sys_nsec_t1 =  timespec_to_ns(&ts1);
				bus_delay->sys_nsec_t4 =  timespec_to_ns(&ts4);
			}
			if( (timespec_compare(&ts1,&gpio_intr_time ) < 0) && ( timespec_compare(&ts4,&gpio_intr_time ) > 0) )
			{
				bus_delay->sys_nsec_t1 =  timespec_to_ns(&gpio_intr_time);
				bus_delay->sys_nsec_t4 =  1;
			}else
			{
				bus_delay->sys_nsec_t4 =  0;
			}
		}		
	}		
#endif
