...
#if 0//defined(OOB_INTR_ONLY) && !defined(HW_OOB)
...
extern struct timespec gpio_intr_time;
void
dhdsdio_oob_isr(void *arg)
{
	getnstimeofday(&gpio_intr_time);
}
...
int dhd_bus_oob_intr_register(dhd_pub_t *dhdp)
{
	...
	err = bcmsdh_oob_intr_register(dhdp->bus->sdh, dhdsdio_oob_isr, dhdp->bus);
#endif
	return err;
}
