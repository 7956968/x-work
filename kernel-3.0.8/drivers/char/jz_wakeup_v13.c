...
static void wakeup_timer_handler(unsigned long data)
{
	...
	/* reset the timer(30ms) at very beginning. */
	mod_timer(&wakeup->wakeup_timer, jiffies + msecs_to_jiffies(30));

	spin_lock_irqsave(&wakeup->suspend_lock, flags2);
	if ( wakeup->suspend_state == 0) {
		/* */
		if(wakeup_module_process_data() == SYS_WAKEUP_OK) {
			...
			if(wakeup->wakeup_pending == 1) {
				...
				wakeup->wakeup_pending = 0;
				if(wakeup->wakeup_waiting == 1) {
					wake_up(&wakeup->wakeup_wq);
				}
				else {
					/* wait event interrupted by OS, keep wakeup state */
					/* wakeup_module_wakeup_word_match(); */
				}
			}
			spin_unlock_irqrestore(&wakeup->wakeup_lock, flags);
		}
		...
		printk("%s():%d, suspend_state:%d\n", __func__, __LINE__, wakeup->suspend_state);
	}
	spin_unlock_irqrestore(&wakeup->suspend_lock, flags2);
		...
		char *buf)
{
	...
	if (!wakeup->wakeup_enable) {
		printk("%d %s() !wakeup->wakeup_enable\n", __LINE__, __FUNCTION__);
		return 0;
	}

	spin_lock_irqsave(&wakeup->wakeup_lock, flags);
	wakeup->wakeup_waiting = 1;
	//ret = wait_event_interruptible(wakeup->wakeup_wq, wakeup->wakeup_pending == 0);
	ret = wait_event_interruptible_lock_irq(wakeup->wakeup_wq, wakeup->wakeup_pending == 0, wakeup->wakeup_lock);
	wakeup->wakeup_waiting = 0;
	spin_unlock_irqrestore(&wakeup->wakeup_lock, flags);
	...
	if(ret && wakeup->wakeup_pending == 1) {
		...
		/*
		 * when called this function. means wakeup is pending now.
		 * set wakeup_pending before another reading.
		 * */
		spin_lock_irqsave(&wakeup->wakeup_lock, flags);
		wakeup->wakeup_pending = 1;
		//printk("%d %s() set wakeup->wakeup_pending = 1;\n", __LINE__, __FUNCTION__);
		spin_unlock_irqrestore(&wakeup->wakeup_lock, flags);
	}
	return t-buf;
}
...
static int wakeup_suspend(void)
{
	...
	/* voice trigger match during suspend, just return. */
	if(wakeup->wakeup_pending == 0) {
		printk("%d %s() wakeup->wakeup_pending == 0; do not return -1; return 0;\n", __LINE__, __FUNCTION__);
		spin_unlock_irqrestore(&wakeup->wakeup_lock, flags);
		return 0;
	}
	...
	spin_lock_irqsave(&wakeup->suspend_lock, flags);
	/* mutex lock? */
	wakeup->suspend_state = 1;
	spin_unlock_irqrestore(&wakeup->suspend_lock, flags);
...
static void wakeup_resume(void)
{
	...
	/* cancel sleep_buffer */
	wakeup_module_set_sleep_buffer(NULL);
	...
	if(wakeup_module_is_cpu_wakeup_by_dmic()) {
		...
		if(wakeup->wakeup_pending == 1) {
			wakeup->wakeup_pending = 0;
			wake_up(&wakeup->wakeup_wq); /* Actually, wait_event already interrupted by OS */
		}
		spin_unlock_irqrestore(&wakeup->wakeup_lock, flags);
	}

	spin_lock_irqsave(&wakeup->suspend_lock, flags);
	wakeup->suspend_state = 0;
	spin_unlock_irqrestore(&wakeup->suspend_lock, flags);
}
...
static int __init wakeup_init(void)
{
	...
	spin_lock_init(&wakeup->suspend_lock);
