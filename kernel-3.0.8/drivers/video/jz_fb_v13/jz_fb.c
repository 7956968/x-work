...
#elif CONFIG_PRODUCT_X1000_HALLEY2
#include "logo_bmp_halley2.h"
#elif CONFIG_PRODUCT_X1000_JS1
#include "logo_bmp_aslmom.h"
#else
#include "logo_bmp.h"
#endif
...
struct bl_pwr_pin __attribute__((weak)) bl_pwr_en_pin = {
        .num = -1,
        .enable_level = -1,
};
...
static void jzfb_slcd_mcu_init(struct fb_info *info)
{
	...
	if (pdata->smart_config.slcd_clear) {
		int h;
		while(pdata->modes->yres--) {
			for(h= 0;h < pdata->modes->xres;h++) {
				slcd_send_mcu_data(jzfb, 0x00);
				slcd_send_mcu_data(jzfb, 0x00);
				slcd_send_mcu_data(jzfb, 0x00);
			}
		}
		if (pdata->smart_config.display_on) {
			slcd_send_mcu_command(jzfb,
				pdata->smart_config.display_on);
			udelay(5000);
		}
	}
	...
	if(pdata->bpp / pdata->smart_config.bus_width != 1 ) {
		...
		if (pdata->smart_config.datatx_type_serial
			...
			tmp |=  (pdata->bpp / pdata->smart_config.bus_width)  == 2 ?
...
static int __devinit jzfb_probe(struct platform_device *pdev)
{
	...
	jzfb->pdata->bl_en = &bl_pwr_en_pin;

	mutex_init(&jzfb->lock);
	mutex_init(&jzfb->suspend_lock);

	if (jzfb->pdata->bl_en->num >= 0) {
		if (gpio_request(jzfb->pdata->bl_en->num, "backlight-enable")) {
			dev_err(&pdev->dev, "No backlight enable pin available\n");
			ret = -ENODEV;
	...
	*(volatile unsigned int*)0xb0000020 &= ~(1 << 23);*/
	...
	if ((jzfb->pdata->bl_en->num >= 0) && (jzfb->pdata->bl_en->enable_level >= 0))
		gpio_direction_output(jzfb->pdata->bl_en->num,
			jzfb->pdata->bl_en->enable_level);
...
static int __devexit jzfb_remove(struct platform_device *pdev)
{
	...
	gpio_free(jzfb->pdata->bl_en->num);
