...
#define USE_NEW_SETTING_FROM_OV // config from omnivision at 12.7.2016, this effect is better than before.
...
#ifdef USE_NEW_SETTING_FROM_OV
ov_camera reg_ov7725_init[] =
{
    {0x12, 0x80},
...
static enum v4l2_mbus_pixelcode ov772x_codes[] = {
...
static const struct ov772x_color_format ov772x_cfmts[] = {
	{
		...
		.dsp3		= UV_OFF,
		.dsp4		= DSP_OFMT_YUV,
		.com3		= 0x00,				/* bug in here, recovery by Jaly */
		.com7		= OFMT_YUV,
	},
	{
...
static int ov7725_write_array(struct i2c_client *client,
			      const ov_camera *vals)
{
	int ret;

	while (vals->regAddr != 0xff) {
		ret = ov772x_write(client, vals->regAddr, vals->regVal);
		dev_vdbg(&client->dev, "array: 0x%02x, 0x%02x",
			 vals->regAddr, vals->regVal);
		printk("regAddr: 0x%x, regVal: 0x%x, ", vals->regAddr, vals->regVal);
		if (ret < 0)
			return ret;
		vals++;
	}
...
static int ov772x_init(struct v4l2_subdev *sd, u32 val)
{
	int ret;
	struct i2c_client  *client = v4l2_get_subdevdata(sd);
...
#ifdef USE_NEW_SETTING_FROM_OV
	ret = ov7725_write_array(client, &reg_ov7725_init);
#else
	ret = ov7725_write_array(client, &reg_ov7725);
#endif
	if (ret < 0)
		goto err;
			     ...
			     const struct ov772x_win_size *win)
{
	...
	if (ret < 0)
		goto ov772x_set_fmt_error;
	ret = ov772x_write(client, CLKRC, 0x01);				/* div =  (1 + 1)* 2 */
	if (ret < 0)
		goto ov772x_set_fmt_error;
...
#define OFFSET_PIX		0x03
	...
	if (ret < 0)
		goto ov772x_set_fmt_error;
	ret = ov772x_write(client, DSP_CTRL3, 0x10);	/* 0x80 | 0x10 */
	if (ret < 0)
		goto ov772x_set_fmt_error;

	/* polariry setting */
	ret = ov772x_write(client, COM10, 0x00 | 0x02 | 0x10);	/* plck reverse */
	if (ret < 0)
		goto ov772x_set_fmt_error;
...
static int ov772x_enum_fmt(struct v4l2_subdev *sd, unsigned int index,
			   enum v4l2_mbus_pixelcode *code)
{
	if (index >= ARRAY_SIZE(ov772x_codes))
		return -EINVAL;

	*code = ov772x_codes[index];
	return 0;
}
			...
			const struct i2c_device_id *did)
{
	...
	if (!icd) {
		dev_err(&adapter->dev, "OV772X: missing soc-camera data!\n");
		return -EINVAL;
	}
	...
	if (!icl) {
		dev_err(&adapter->dev,
			"OV772X: Missing platform_data for driver\n");
		return -EINVAL;
	}
