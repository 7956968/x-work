/*
 * V4L2 Driver for camera sensor gc2155
 *
...
static struct gc2155_win_size gc2155_supported_win_sizes[] = {
		...
		struct v4l2_frmsizeenum *fsize)
{
	...
	for (i = 0; i < N_WIN_SIZES; i++) {
		struct gc2155_win_size *win = &gc2155_supported_win_sizes[index];
		if (index == ++num_valid) {
...
static int gc2155_video_probe(struct soc_camera_device *icd,
			      struct i2c_client *client)
{
	 ...
	 */
	retval = i2c_smbus_read_byte_data(client, 0xf0);
	retval <<= 8;
	retval |= i2c_smbus_read_byte_data(client, 0xf1);
			...
			const struct i2c_device_id *did)
{
	...
	if(!strcmp(client->name, "gc2155-back")) {
		client->addr -= 1;
	}

	if (!icd) {
		dev_err(&adapter->dev, "GC2155: missing soc-camera data!\n");
		return -EINVAL;
	}
	...
	if (!icl) {
		dev_err(&adapter->dev,
			"GC2155: Missing platform_data for driver\n");
		return -EINVAL;
	}
	...
	ret = gc2155_video_probe(icd, client);
	if (ret) {
		icd->ops = NULL;
		kfree(priv);
	}
