...
static int audio_buf_size = SAMPLE_RATE;
...
static struct uac_feature_unit_descriptor_0 spk_feature_unit_desc = {
	...
	.bControlSize		= 1,//(0 + 1) * sizeof(__le16),
...
static int mic_interface_conn(struct usb_function * f)
{
	...
	int ret = usb_ep_enable(audio->in_ep, audio->in_desc);
	if (ret)
		return ret;

	audio->in_online = 1;
#ifdef CONFIG_CAP_FILE
	audio->mic_disconn = 0;
	wake_up(&audio->online_wq);
#endif
#ifdef CONFIG_CAP_MIC
...
static LIST_HEAD(cb_free_list);

struct cb_node {	/*capture buffer*/
...
static struct cb_node* alloc_cb_node(struct usb_ep *ep, size_t size, void *context)
{
	...
	cb_node = kmalloc(size + sizeof(cb_node), GFP_ATOMIC);
	cb_node->req = usb_ep_alloc_request(ep, GFP_ATOMIC);
	cb_node->req->buf = (void *)(cb_node + 1);
	cb_node->req->length = size;
	cb_node->req->complete = capture_complete;
	cb_node->req->context = context;
	INIT_LIST_HEAD(&cb_node->node);
	list_add_tail(&cb_node->free, &cb_free_list);
	return cb_node;
}

static void __clear_cb_list(struct f_audio *audio)
{
	struct cb_node *cb_node, *tmp;
	unsigned long flags;
	...
	list_for_each_entry_safe(cb_node, tmp, &cb_free_list, free) {
		list_del(&cb_node->free);
...
static int audio_capture_thread(void *data)
{
	...
	while (!wait_for_capture(audio)) {
		...
		ret = usb_ep_queue(audio->in_ep, cb_node->req, GFP_KERNEL);
		if (ret < 0) {
			if (ret == -ESHUTDOWN && !audio->in_online)
				continue;
			printk("audio usb in ep queue failed %d\n", ret);
			break;
		}
...
#if defined(CONFIG_CAP_MIC)
	kthread_stop(audio->capture_thread);
#endif
