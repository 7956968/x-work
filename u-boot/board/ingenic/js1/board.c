...
{
	int ret;
#ifdef CONFIG_PMU_RICOH618
	ret = ricoh618_regulator_init();
#endif
	return ret;
}
...
int checkboard(void)
{
	puts("Board: Gs1 (Ingenic XBurst X1000 SoC)\n");
	return 0;
}
 ...
 */
extern void *spl_get_nvinfo(void);
int spl_get_boot_type(void)
{
	...
	if (nvinfo)
		if (nvinfo->update_flag == FLAG_UPDATE)
			if (nvinfo->update_process == PROCESS_2)
				return 1;

	//set PC(22),USB_DETE PIN as input
	gpio_port_direction_input(2, 22);

	usb_insert = !gpio_get_value(86);
