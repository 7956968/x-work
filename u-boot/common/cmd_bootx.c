...
static unsigned int trg;
static unsigned int cont = 0;
static unsigned long start_press;
...
static void keyproc(void)
{
	...
	else if ((trg == 1) && (cont ==1)) {
		shortpress = 1;
		longpress = 0;
	}
	else if ((trg == 0) && (cont ==1)) {
		shortpress = 1;
		longpress = 1;
	}
	if (shortpress && !longpress)
		start_press = get_timer(0);
	if (longpress) {
		if (get_timer(start_press) > timeout) {
			alarm = 1;
			printf("2=>>>>>get_timer(start_press)=%ld\n",get_timer(start_press));
		}
	}
}
...
void display_battery_capacity(int line)
{
	int i;
	for(i = 1;i <= line; i++){
#if defined CONFIG_ASLMOM_BOARD
		if(!gpio_get_value(63) ||(gpio_get_value(40)))
#elif defined CONFIG_JS1_BOARD
		if(!gpio_get_value(63) ||(gpio_get_value(86)))
#endif
			return;
